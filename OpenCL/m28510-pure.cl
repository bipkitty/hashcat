...
KERNEL_FQ void m28510_init (KERN_ATTR_TMPS (bip39_tmp_t))
{
  ...
  // printf_array(mnemonic, mnemonic_encoder.len);
  // printf_array(passphrase, passphrase_encoder.len);
  // printf("\n");
}
...
KERNEL_FQ void m28510_comp (KERN_ATTR_TMPS (bip39_tmp_t))
{
  ...
  // if (r0 == digests_buf[DIGESTS_OFFSET_HOST].digest_buf[0]) {
  // printf("\n\nMaster seed: ");
  // for (int i = 0; i < 8; i ++) {
  // printf("%016lx", tmps[gid].out[i]);
  // }

  // printf("\nRvalue: %08x %08x %08x %08x", r0, r1, r2, r3);
  // printf("\nDigest: %08x %08x %08x %08x", digests_buf[DIGESTS_OFFSET_HOST].digest_buf[0], digests_buf[DIGESTS_OFFSET_HOST].digest_buf[1], digests_buf[DIGESTS_OFFSET_HOST].digest_buf[2], digests_buf[DIGESTS_OFFSET_HOST].digest_buf[3]);
  // printf("\n");
  // }
