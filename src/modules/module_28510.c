...
static const address_base58_t XPUB_ADDRESS = { 82, 13, {0x04, 0x88, 0xb2, 0x1e, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF} };
static const address_base58_t P2PKH_ADDRESS = { 25, 1, {0x0} };
static const address_base58_t P2SHWPKH_ADDRESS = { 25, 1, {0x05} };
...
DECLSPEC u32 decode_mnemonic_phrase (PRIVATE_AS u8 ** mnemonic_phrase, PRIVATE_AS const char end_char)
{
  ...
  u32 parsed = hc_strtoul (word, 0, 10);
  if (parsed)
    return parsed | display;

  return bip39_from_word (word) | display;
}

// Helper for base58 decoding into the digest, assumes prefix ends in 0xFF
static bool decode_base58 (const address_base58_t * type, const char *address, u8 * digest, const u32 address_len)
{
  u32 digest_len = type->digest_len;
  u32 prefix_len = type->prefix_len;
  u8 decoded_address[128] = { 0 };

  if (!b58dec (decoded_address, &digest_len, (u8 *) address, address_len))
    return false;

  if (!b58check (decoded_address, digest_len))
    return false;
  ...
  for (u32 i = 0; i < digest_len - prefix_len; i++)
  {
...
int module_hash_decode (MAYBE_UNUSED const hashconfig_t * hashconfig, MAYBE_UNUSED void *digest_buf, MAYBE_UNUSED salt_t * salt, MAYBE_UNUSED void *esalt_buf, MAYBE_UNUSED void *hook_salt_buf, MAYBE_UNUSED hashinfo_t * hash_info, const char *line_buf, MAYBE_UNUSED const int line_len)
{
  ...
  if (strncmp (address_type, "XPUB", token.len[0]) == 0)
  {
    ...
    if (!decode_base58 (&XPUB_ADDRESS, address, digest, token.len[3]))
      return PARSER_HASH_ENCODING;
  }
  else if (strncmp (address_type, "P2PKH", token.len[0]) == 0)
  {
    salt->salt_buf[0] = P2PKH_ADDRESS_ID;
    if (!decode_base58 (&P2PKH_ADDRESS, address, digest, token.len[3]))
      return PARSER_HASH_ENCODING;
  }
  else if (strncmp (address_type, "P2SH-P2WPKH", token.len[0]) == 0)
  {
    salt->salt_buf[0] = P2SHWPKH_ADDRESS_ID;
    if (!decode_base58 (&P2SHWPKH_ADDRESS, address, digest, token.len[3]))
      return PARSER_HASH_ENCODING;
  }
  else if (strncmp (address_type, "P2WPKH", token.len[0]) == 0)
  {
...
int module_build_plain_postprocess (MAYBE_UNUSED const hashconfig_t * hashconfig, MAYBE_UNUSED const hashes_t * hashes, MAYBE_UNUSED const void *tmps, const u32 * src_buf, MAYBE_UNUSED const size_t src_sz, MAYBE_UNUSED const int src_len, u32 * dst_buf, MAYBE_UNUSED const size_t dst_sz)
{
  ...
  for (u32 i = salt_index; salt[salt_index] != MNEMONIC_END; salt_index++)
  {
    if (salt[salt_index] == MNEMONIC_GUESS)
    {
      ...
      encode_char (&encoder, ',');
    } else if (salt[salt_index] > MNEMONIC_DISPLAY)
    {
      encode_mnemonic_word (&encoder, words[salt_index - i] & 0X00FFFFFF);
      encode_char (&encoder, ',');
    }
  }
